...
$pets = $conn->query("SELECT * FROM pets WHERE type='dog'");
?>
...
<div class="pet-container">
    ...
    <div class="pet-card">
        ...
        <h2>Name: <?= htmlspecialchars($pet['name']) ?></h2>
        <h2>Price: <?= number_format($pet['price'], 2) ?></h2>
        <h2>Suggested Food: <?= htmlspecialchars($pet['food']) ?></h2>

        <!-- Add to Cart -->
        <button class="add-to-cart"
            onclick="window.location.href='cart.php?id=<?= $pet['id'] ?>&name=<?= urlencode($pet['name']) ?>&price=<?= $pet['price'] ?>&image=<?= urlencode($pet['image']) ?>'">
            Add to Cart
        </button>

        <!-- Buy Now -->
        <button class="buy-now"
            onclick="window.location.href='buyNow.php?id=<?= $pet['id'] ?>&name=<?= urlencode($pet['name']) ?>&price=<?= $pet['price'] ?>&image=<?= urlencode($pet['image']) ?>'">
            Buy Now
        </button>
    </div>
    <?php } ?>
